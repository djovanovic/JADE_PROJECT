...
public class OrderBookingEngine {
	...
	private ArrayList<OrderBookEntry> createOrder(String orderID, OrderBookEntryType type, int quantity, int price) { 
		
		return new ArrayList<OrderBookEntry>(Arrays.asList(
				new OrderBookEntry(orderID, type, quantity, price)));
	}
	
	ArrayList<OrderBookEntry> temp;
	
	protected boolean book(String orderID, OrderBookEntryType type, int quantity, int price) {
		...
		while (iterator.hasNext()) {
			...
			if (nextBestPrice == price) {
				temp.add(new OrderBookEntry(orderID, type, quantity, price));
				return true;
				
			} else if ((nextBestPrice > price)) {
				
				orderBookList.add(iterator.previousIndex(),
						createOrder(orderID, type, quantity, price));
				return true;
				...
				orderBookList.addLast(createOrder(orderID, type, quantity, price));
				return true;
			}
		}
		...
		if (orderBookList.size() == 0) {
			...
			orderBookList.addFirst(createOrder(orderID, type, quantity, price));
			return true;
		}
